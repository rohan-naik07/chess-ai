...
export const checkConstraints = position=>{
    return (
        position[0] >= 0 && position[0]<8 &&
        position[1] >= 0 && position[1]<8 
    )};

export const checkOverlap = (id,selectedLocation,positions)=>{
...
export default class verifyCheckMate {
    ...
    generateMoves(turn,positions){
        ...
        Object.keys(positions).forEach(key=>{
            if(positions[key].substring(0,5)===turn){
                let temp = this.getMoves(key,positions[key].substring(5),positions);
    ...
    isCheckmated(depth,turn,positions){
        let flag=0;
        this.count+=1;
